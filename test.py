...
def move(distance):
    ...
    try:
        ...
        # 移動量を設定（100mm）
        moveAMT = 100.0  # mm
        posAMove = moveAMT * (1/cfA)  # Galil内部単位に変換
        ...
        print(f'Moving A-axis {distance}mm ({moveAMT}mm) to position {posAMove} units')
        logging.info(f'Moving A-axis {distance}mm ({moveAMT}) to position {posAMove} units')
